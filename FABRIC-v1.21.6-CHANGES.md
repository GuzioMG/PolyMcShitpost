...
Continuing with our conventional tag api, We added new biome tags, allowing modders to differentiate biomes based on their primary wood type. The `ServerChunkEvents.CHUNK_LEVEL_TYPE_CHANGE` was added to allow more control over the timing of chunk events. This event fires for changes in chunk loading level, to react to changes not previously possible without mixins. ([#4541](https://github.com/FabricMC/fabric/pull/4541))
...
## PolyMc-specific changes for 1.21.6

### Network Packet Changes

The most significant change is that `PacketCallbacks` has been replaced with `ChannelFutureListener` in network send methods. All mixins that inject into packet sending methods need to update their signatures:
...
The PacketCodecs anonymous inner classes have changed their structure and numbering. While we've attempted to update these mixins, they are currently disabled due to method signature mismatches. The encode methods now follow a different pattern and the anonymous class numbers have shifted (e.g., $19 → $18, $21 → $20, $33 → $32).
...
1. **FallingBlockEntityMixin**: Updated to use simpler method descriptors for `onStartedTrackingBy` and `onStoppedTrackingBy`
2. **Network Handler Mixins**: Updated all packet send injections to use `ChannelFutureListener`

The mod builds successfully with these changes, though runtime testing reveals some compatibility issues with dependencies that need to be addressed as they update to 1.21.6.